...
class Expander:
        ...
        for index in range(len(selected_nodes)):
            ...
            for node in com[1:]:
                if node != 'EOS':
                    ...
                    pre_cond = self.calculate_conductance_scipy(temp_com)
    ...
    def calculate_conductance_scipy(self, community_nodes):
        ...
        # Ensure nodes are valid indices
        valid_nodes = [n for n in community_nodes if n < self.graph.adj_mat.shape[0]]
        if not valid_nodes:
            return 1.0
        ...
        # Adjacency
        adj = self.graph.adj_mat
        
        # Volume(S) = sum of degrees of nodes in S
        # adj is scipy sparse matrix
        degrees = np.array(adj.sum(axis=1)).flatten()
        vol_S = np.sum(degrees[node_indices])
        
        # Volume(V)
        vol_total = np.sum(degrees)
        
        # Cut(S, V\S) = Vol(S) - (2 * edges_inside_S) ? 
        # For undirected graph: sum(deg(u) for u in S) = 2*|E_in| + |E_out|
        # So |E_out| = Vol(S) - 2*|E_in|
        
        # edges inside S
        # submatrix of S
        sub_adj = adj[node_indices, :][:, node_indices]
        # For undirected graph, sub_adj.sum() counts each internal edge twice.
        vol_in = sub_adj.sum() 
        
        cut_S = vol_S - vol_in
        
        vol_bar_S = vol_total - vol_S
        denominator = min(vol_S, vol_bar_S)
        
        if denominator == 0:
            return 1.0 # Worst case
            
        return cut_S / denominator
